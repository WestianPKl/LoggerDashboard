...
export default function PCBBomTable({ pcbId, pcbBomItems, initSort, initFilter }: IPCBBomTableProps) {
	...
	const columns = useMemo<GridColDef[]>(
		() => [
			...
			{ field: 'inventory', headerName: 'Nazwa', width: 200, valueGetter: (_, row) => row.inventory?.name ?? '-' },
			{
			...
			{
				field: 'valueSpec',
				headerName: 'Specyfikacja wartości',
				width: 200,
				valueGetter: (_, row) => row.valueSpec ?? '-',
			},
			{
				field: 'allowSubstitution',
				headerName: 'Substytucja',
				width: 150,
				valueGetter: (_, row) => (row.allowSubstitution ? 'Tak' : 'Nie'),
			},
