...
export default function InventoryShopTable({ inventoryShop, initSort, initFilter }: IInventoryShopTableProps) {
	...
	const [addInventoryShop] = useAddInventoryShopMutation()
	...
	const inventoryShopMap = useMemo(() => {
		const map = new Map()
		inventoryShop.forEach(item => {
	...
	const selectedItems = useMemo(() => {
		...
		return selectedIds.map(id => inventoryShopMap.get(Number(id))).filter(Boolean) as InventoryShopClass[]
	}, [rowSelectionModel, inventoryShopMap])

	useEffect(() => {
		localStorage.setItem('inventoryTableShopSortModel', JSON.stringify(sortModel))
	}, [sortModel])

	useEffect(() => {
		localStorage.setItem('inventoryTableShopFilterModel', JSON.stringify(filterModel))
	}, [filterModel])
	...
	async function addItemHandler(item: IAddInventoryAdditionalData | IAddInventoryAdditionalData[]): Promise<void> {
		try {
			...
			if (!Array.isArray(item)) {
				await addInventoryShop(item).unwrap()
			}
			dispatch(showAlert({ message: 'New inventory Shop added', severity: 'success' }))
			...
			if (Array.isArray(items) && items.length >= 1) {
				await Promise.all(
					items.map(async item => {
						await updateInventoryShop(item).unwrap()
					}),
				)
				dispatch(showAlert({ message: 'Inventory Shop edited', severity: 'success' }))
				clearObject()
				revalidator.revalidate()
			}
		} catch (err: any) {
			...
			if (selectedItems.length >= 1) {
				await Promise.all(
					selectedItems.map(async item => {
						await deleteInventoryShop({ id: item.id }).unwrap()
					}),
				)
				dispatch(showAlert({ message: 'Inventory Shop deleted', severity: 'success' }))
				revalidator.revalidate()
			}
		} catch (err: any) {
			const message = err?.data?.message || err?.message || 'Something went wrong'
