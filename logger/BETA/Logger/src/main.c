...
static volatile uint8_t wait_adc = 0;
...
static void stm32_read_adc(void) {
    uart_send(CMD_ADC, 0x00, NULL, 0);
    wait_adc = 1;
}
        ...
        if (rx_ready) {
           ...
           if (wait_fw_hw_version && rx_frame[2] == CMD_SERIAL && rx_frame[3] == FW_HW_VERSION_READ) {
            ...
            if (wait_fw_build && rx_frame[2] == CMD_SERIAL && rx_frame[3] == FW_BUILD_READ) {
                ...
                memcpy(g_stm32_data.fw_build, (const void *)&rx_frame[4], 10);
                g_stm32_data.fw_build[10] = '\0';

                stm32_read_adc();
            }

            if (wait_adc && rx_frame[2] == CMD_ADC) {
                wait_adc = 0;

                uint16_t adc_mv =
                    ((uint16_t)rx_frame[4] << 8) |
                    rx_frame[5];
                g_bme.adc_v = adc_mv * 3.3 / 4095.0;
            ...
            if (wait_production_date && rx_frame[2] == CMD_SERIAL && rx_frame[3] == PRODUCTION_DATE_READ) {
                ...
                memcpy(g_stm32_data.production_date, (const void *)&rx_frame[4], 10);
                g_stm32_data.production_date[10] = '\0';
