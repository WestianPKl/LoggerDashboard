...
STATUS_GPIO = 18  # INPUT to be set by STM32 to indicate status (e.g. error)
STM32_GPIO = (
    19  # OUTPUT to be set by ESP32 to indicate status - read by STM32+ (e.g. error)
)
LED_GPIO = 22
...
def uart_loop_application(uart: UART, frame: bytes) -> bytes | None:
    ...
    while time.ticks_diff(time.ticks_ms(), t0) < 1500:
        ...
        rest = read_exact(uart, FRAME_LEN_APP - 1, 1200)
        if rest is None:
            return None

        data = bytes([DEV_ADDR]) + rest
        if crc8_atm(data[: FRAME_LEN_APP - 1]) != data[FRAME_LEN_APP - 1]:
            continue
        return data
    return None
...
def vadc_to_vin(vadc, r_top=200_000.0, r_bottom=68_000.0):
...
def req_adc(uart: UART):
    ...
    print("ADC0 voltage:", adc_to_voltage(v0), "V")
    print("ADC1 voltage:", adc_to_voltage(v1), "V")
    print("ADC0 Vin:", vadc_to_vin(adc_to_voltage(v0)), "V")
    print("ADC1 Vin:", vadc_to_vin(adc_to_voltage(v1)), "V")
...
def req_bme280(uart: UART):
    resp = uart_loop_application(uart, uart_message_application(0x03, 0x01))
    parsed = parse_resp(resp)
    if not parsed:
...
def req_get_input_states(uart: UART, channel: int):
    resp = uart_loop_application(uart, uart_message_application(0x02, channel))
    parsed = parse_resp(resp)
    if not parsed:
        print(f"INPUT {channel}: no response")
        return
    addr, status, cmd, param, payload, crc = parsed
    if status != STATUS_OK:
        print(f"INPUT {channel}: error")
        return
    print(f"INPUT {channel} state:", payload[0])
...
def req_set_output(uart: UART, channel: int, on: int):
    resp = uart_loop_application(
        uart, uart_message_application(0x04, channel, bytes([on & 0xFF]))
    )
    parsed = parse_resp(resp)
    if not parsed:
        print(f"OUTPUT {channel}: no response")
        return
    addr, status, cmd, param, payload, crc = parsed
    if status != STATUS_OK:
        print(f"OUTPUT {channel}: error")
        return
    print(f"OUTPUT {channel} set to:", payload[0])


def req_set_pwm(uart: UART, channel: int, duty: int):
    resp = uart_loop_application(
        uart, uart_message_application(0x05, channel, bytes([duty & 0xFF]))
    )
    parsed = parse_resp(resp)
    if not parsed:
        print(f"PWM {channel}: no response")
        return
    addr, status, cmd, param, payload, crc = parsed
    if status != STATUS_OK:
        print(f"PWM {channel}: error")
        return
    print(f"PWM {channel} set to:", payload[0])


def req_set_rgb(uart: UART, r: int, g: int, b: int):
    resp = uart_loop_application(
        uart,
        uart_message_application(0x05, 0x05, bytes([r & 0xFF, g & 0xFF, b & 0xFF])),
    )
    parsed = parse_resp(resp)
    if not parsed:
...
def req_set_buzzer(uart: UART, freq: int, volume: int):
    ...
    resp = uart_loop_application(uart, uart_message_application(0x05, 0x06, pl))
    parsed = parse_resp(resp)
    if not parsed:
...
def main():
    ...
    input_channels = [
        0x01,  # BTN1,
        0x02,  # BTN2,
        0x03,  # ESP32_STATUS (read by STM32 to check if ESP32 is alive),
    ]
    for ch in input_channels:
        if ch == 0x03:
            print("Simulating ESP32 alive...")
            stm32_gpio.value(1)  # Set status high to indicate ESP32 is alive
            req_get_input_states(uart, ch)
            time.sleep(1)
            print("Simulating ESP32 not alive...")
            stm32_gpio.value(0)  # Set status low to indicate ESP32 is not alive
            req_get_input_states(uart, ch)
        else:
            req_get_input_states(uart, ch)
            time.sleep(1)
    ...
    for ch, name in channels:
        req_set_output(uart, ch, 1)
        if name == "ESP32_STATUS":
            print(
                "Status GPIO (STM32->ESP32):",
                "LOW" if status_gpio.value() == 0 else "HIGH",
            )
        time.sleep(1)
        req_set_output(uart, ch, 0)
        if name == "ESP32_STATUS":
            print(
                "Status GPIO (STM32->ESP32):",
                "LOW" if status_gpio.value() == 0 else "HIGH",
    ...
    for ch, name in pwm_channels:
        for duty in [0, 1, 64, 128, 192, 254, 255, 0]:
            req_set_pwm(uart, ch, duty)
            time.sleep(0.5)
    ...
    #     req_set_buzzer(uart, freq, 50)
    #     time.sleep(0.2)
    # req_set_buzzer(uart, 0, 0)

    # 0x06 - RTC
    date_time = rtc.datetime()
    print(date_time[0] - 2000, date_time[1], date_time[2], date_time[3], date_time[4], date_time[5], date_time[6])
    req_rtc_read(uart)
    req_rtc_write(uart, date_time[0] - 2000, date_time[1], date_time[2], date_time[3], date_time[4], date_time[5], date_time[6])
    req_rtc_read(uart)
