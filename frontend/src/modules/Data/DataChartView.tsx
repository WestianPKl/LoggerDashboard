...
export async function loader({ params }: LoaderFunctionArgs) {
	const equLoggerId = params.equLoggerId
	if (!equLoggerId) {
		throw data('No logger Id', { status: 400 })
	}
	try {
		const promiseConnectedSensors = await store
			.dispatch(dataApi.endpoints.getDataConnectedSensorView.initiate({ equLoggerId: equLoggerId }))
			.unwrap()
		const promiseEquipments = await store
			.dispatch(equipmentApi.endpoints.getEquipments.initiate({ id: equLoggerId }))
			.unwrap()
		...
		return {
			...
			equLoggerId: parseInt(equLoggerId),
		}
	} catch (err: any) {
		store.dispatch(
			showAlert({
